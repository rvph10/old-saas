...
describe('MailerService', () => {
  ...
  describe('Redis caching for Ethereal account', () => {
    ...
    it('should create new account if cache is invalid', async () => {
      ...
      (nodemailer.createTestAccount as jest.Mock).mockResolvedValueOnce(newAccount);
      mockConfigService.get.mockReturnValue('development');
  ...
  describe('Email Templates', () => {
    ...
    describe('sendEmailVerification', () => {
      ...
      it('should include correct verification link', async () => {
        ...
        expect(mockTransporter.sendMail).toHaveBeenCalledWith(
          expect.objectContaining({
            html: expect.stringContaining(`${mockConfigService.get('FRONTEND_URL')}/verify-email?token=${token}`)
          })
        );
      });
    });
  ...
  describe('Error Handling', () => {
    it('should handle SMTP connection errors', async () => {
      ...
      await expect(service.onModuleInit()).rejects.toThrow('SMTP connection failed');
    });
    ...
    it('should handle errors during Ethereal account creation', async () => {
      ...
      await expect(newService.onModuleInit()).rejects.toThrow('Failed to initialize Ethereal account');
    });
    ...
    it('should handle errors during Ethereal account verification', async () => {
      ...
      mockRedisService.get.mockResolvedValueOnce(JSON.stringify(cachedAccount));
      mockTransporter.verify.mockRejectedValueOnce(new Error('Ethereal account verification failed'));

      await expect(service.onModuleInit()).rejects.toThrow('Ethereal account verification failed');
    });
  });
  ...
  describe('Development Environment Logging', () => {
    describe('sendEmailVerification', () => {
      it('should log email details in development environment', async () => {
        ...
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('Preview URL:'));
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('MessageId:'));
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('To: test@example.com'));
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('Subject: Verify Your Email'));
      });
    });
    ...
    describe('sendPasswordReset', () => {
      it('should log email details in development environment', async () => {
        ...
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('MessageId:'));
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('To: test@example.com'));
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('Subject: Password Reset Request'));
      });
    });
    ...
    describe('sendWelcome', () => {
      it('should log email details in development environment', async () => {
        ...
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('MessageId:'));
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('To: test@example.com'));
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('Subject: Welcome to Nibblix'));
      });
    });
  });
});